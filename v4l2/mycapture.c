...
static void jpegWrite(unsigned char* img)
{
  ...
  FILE *outfile = fopen( "test.jpg", "wb" );
  ...
  jpeg_set_quality(&cinfo, 2/*jpegQuality*/, TRUE);
...
static void yuv2rgb(int width,int height,unsigned char *src, unsigned char *dst){
	int line,column;
	unsigned char *py,*pu,*pv;
	unsigned char *tmp = dst;
	py = src;pu=src+1;pv+3;
	for(line = 0;line<height;++line){
		for(column = 0;column<width;++column){
			*tmp++ = (double)*py+1.402*((double)*pv-128.0);
			*tmp++ = (double)*py - 0.344*((double)*pu-128.0) - 0.714*((double)*pv-128.0);
			*tmp++ = (double)*py + 1.772*((double)*pu-128.0);
			py +=2;
			if((column&1)==1){
				pu+=4;
				pv+=4;
